...
export default function Suksesi() {
  ...
  const { data: plans = [] } = useQuery<any[]>({ queryKey: ["hr-succession"], queryFn: () => fetch("/api/hr/succession").then(apiJson) });

  const save = useMutation({
    mutationFn: (body: any) => editId
      ? fetch(`/api/hr/succession/${editId}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).then(apiJson)
      : fetch("/api/hr/succession", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).then(apiJson),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["hr-succession"] }); resetForm(); },
  });

  const del = useMutation({
    mutationFn: (id: number) => fetch(`/api/hr/succession/${id}`, { method: "DELETE" }).then(apiJson),
